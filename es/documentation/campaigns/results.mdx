---
title: "Interpretación de Resultados de Campañas"
...
Los contactos pueden tener los siguientes estados (`status`):

| Estado      | Descripción                                           | Canal    |
| ----------- | ----------------------------------------------------- | -------- |
| `queue`     | En cola, esperando ser procesado                      | Ambos    |
| `sending`   | Enviando mensaje o iniciando llamada                  | Ambos    |
| `failed`    | Fallo general en el envío/llamada                     | Ambos    |
| `delivered` | Mensaje entregado / Llamada contestada por humano     | WhatsApp |
| `read`      | Mensaje leído                                         | WhatsApp |
| `accepted`  | Contacto aceptó la conversación                       | WhatsApp |
| `rejected`  | Contacto rechazó la conversación                      | WhatsApp |
| `voicemail` | Llamada contestada por buzón de voz                   | Llamadas |
| `answered`  | Llamada contestada                                    | Llamadas |
| `retrying`  | En reintento después de estado fallido o no respuesta | Ambos    |
| `converted` | Contacto marcado como convertido por el sistema       | Ambos    |

<Info>
  **Condicional:** Estos estados pueden cambiar a `queue` o `retrying` si el sistema determina que hay reintentos disponibles según la configuración de la campaña.
</Info>
