...
| `status`        | `string` | Status of the call enum value (e.g., `completed`).                                                              |
...
Each request includes an HMAC signature in the `X-Altur-Signature` header. Use this signature to verify the authenticity of the request. Refer to the [Webhook Overview](https://docs.altur.io/api-reference/webhooks/overview#security) for details on validating webhook requests.
